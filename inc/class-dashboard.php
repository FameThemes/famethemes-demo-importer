...
class Demo_Content_Dashboard
{
    ...
    private $page_slug = 'famethemes-demo-importer';
    ...
    function scripts($hook = '')
    {
        ...
        if (! $load_script) {
            if (strpos($hook, 'famethemes-demo-importer') !== false) {
                $load_script = true;
            }
        }
        ...
        wp_enqueue_style('famethemes-demo-importer', DEMO_CONTENT_URL . 'style.css', false);
        if (! Demo_Contents::php_support()) {
            return;
        }

        wp_enqueue_script('underscore');
        wp_enqueue_script('famethemes-demo-importer', DEMO_CONTENT_URL . 'assets/js/importer.js', array('jquery', 'underscore'));
        wp_enqueue_media();
        ...
        wp_localize_script('famethemes-demo-importer', 'demo_contents_params', array(
...
?>
        <script id="tmpl-demo-contents--preview" type="text/html">
            <div id="demo-contents--preview">
                ...
                <span type="button" class="demo-contents-collapse-sidebar button" aria-expanded="true">
                    ...
                    <span class="collapse-sidebar-label"><?php esc_html_e('Collapse', 'famethemes-demo-importer'); // phpcs:ignore WordPress.Security.EscapeOutput.UnsafePrintingFunction 
                                                            ?></span>
                </span>

                <div id="demo-contents-sidebar">
                    <span class="demo-contents-close"><span class="screen-reader-text"><?php esc_html_e('Close', 'famethemes-demo-importer'); // phpcs:ignore WordPress.Security.EscapeOutput.UnsafePrintingFunction 
                                                                                        ?></span></span>
    ...
    function dashboard()
    {
        if (! current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'famethemes-demo-importer')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        }
        ...
        <div class="wrap demo-contents">
            <h1 class="wp-heading-inline"><?php esc_html_e('Demo Contents', 'famethemes-demo-importer'); ?>
                <?php if (Demo_Contents::php_support()) { ?>
                    <span class="title-count theme-count"><?php echo $number_theme; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
                <?php } ?>
            </h1>
            <?php
            if (Demo_Contents::php_support()) {
            ...
            ?>
                <div class="wp-filter hide-if-no-js">
                    <div class="filter-count">
                        <span class="count theme-count"><?php echo $number_theme; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
                    </div>
                    <ul class="filter-links">
                        <li><a href="<?php echo $link_all; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" class="<?php echo (! $tab) ? 'current' : ''; ?>"><?php esc_html_e('All Demos', 'famethemes-demo-importer'); ?></a></li>
                    </ul>
                </div>
    ...
    function loop_theme($theme)
    {
        ...
        <div class="theme <?php echo ($theme['activate']) ? 'demo-contents--current-theme' : ''; ?>" tabindex="0">
            ...
            <div class="theme-author"><?php sprintf(
                                            /* translators: author name */
                                            __('by %s', 'famethemes-demo-importer'),
                                            $theme['author']
                                        ); ?></div>
            ...
            <div class="theme-actions">
                <a
                    ...
                    href="#"><?php esc_html_e('Start Import', 'famethemes-demo-importer'); ?></a>
            </div>
        </div>
    <?php
    }
    ...
    ?>
        <div class="demo-contents-notice"><?php
                                            ...
                                            printf(__("PHP version is not support. You're using PHP version %s, please upgrade to version 5.6.20 or higher.",  'famethemes-demo-importer'), PHP_VERSION); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                                            ?></div>
    <?php
    }
