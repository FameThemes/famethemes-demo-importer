<?php
/**
 * Demo_Contents singleton — historical entry point for the Legacy track.
 *
 * Carries:
 *   - Singleton instance + `dashboard` / `progress` slot properties that
 *     the legacy `Demo_Content_Dashboard` and `Demo_Contents_Progress`
 *     classes attach themselves onto (so callsites like
 *     `Demo_Contents::get_instance()->dashboard->get_tgmpa_plugins()`
 *     keep working byte-for-byte).
 *   - Static helpers used by the legacy class-progress.php / class-dashboard.php
 *     pipelines: `media_handle_sideload()`, `download_file()`,
 *     `php_support()`, `get_github_repo()`, `url_exists()`.
 *   - The `?demo_contents_export=1` admin endpoint that emits a
 *     `config.json` for the active theme (`generate_config()` +
 *     `export()`), plus the widget-export helpers it depends on.
 *
 * Relocated from the root plugin file when the importer was split into
 * per-theme tracks. Generic track has no need for any of this code —
 * relocating means it never loads on non-legacy installs.
 *
 * @package famethemes-demo-importer
 */
...
class Demo_Contents
{
    ...
    {
        // Only the `?demo_contents_export=1` endpoint is registered here.
        // The Dashboard + Progress instances are attached onto the
        // `dashboard` / `progress` properties by the Legacy bootstrap
        // (`inc/legacy/bootstrap.php`) after their class files load.
        if (is_admin()) {
