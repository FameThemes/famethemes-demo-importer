...
class  Demo_Contents_Progress {
    ...
    function ajax_import(){
        ...
        if ( ! current_user_can( 'import' ) ) {
            wp_send_json_error( __( "You have not permissions to import.", 'famethemes-demo-importer' ) );
        }
        ...
        if ( $doing == 'activate_theme' ) {
            ...
            wp_send_json_success( array( 'msg' => sprintf( __( '%s theme activated', 'famethemes-demo-importer' ), $themes[ $current_theme_slug ]['name'] ) ) );
        }

        if ( $doing == 'checking_resources' ){
            $file_data = $this->maybe_remote_download_data_files( $current_theme );
            if ( ! $file_data || empty( $file_data ) ) {
                wp_send_json_error( sprintf( __( 'Demo data not found for <strong>%s</strong>. However you can import demo content by upload your demo files below.', 'famethemes-demo-importer' ) , $themes[ $current_theme_slug ]['name'] ) );
            } else {
